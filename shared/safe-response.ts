...
const redact = (value: unknown): string => (typeof value === "string" ? value : "Integration Recovery request failed")
  .replace(/Bearer\s+[^\s,;]+/gi, "Bearer [REDACTED]")
  .replace(/(x-api-key\s*[:=]\s*)[^\s,;]+/gi, "$1[REDACTED]")
  .slice(0, 500);

const bodyAsJson = (body: unknown): unknown => {
...
export function makeSafeError(statusCode: number, body: unknown): Error {
...
export async function handleChecksResponse(
  data: N8nItem[],
  ...
  if (statusCode < 200 || statusCode >= 300) throw makeSafeError(statusCode, parsed);
  ...
  if (data.length === 0) return [{ json: parsed }];
